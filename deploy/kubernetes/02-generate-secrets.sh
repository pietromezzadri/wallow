...
#
# CLOUDFLARE_TURN_* below are NOT regenerated by this script -- they're
# long-lived account credentials (a Cloudflare Realtime TURN key ID/API
# token) and a short-lived (24h) generated TURN username/credential pair,
# passed in via env so a re-run of this script doesn't silently drop them.
# This is also why every literal lives in ONE `kubectl create secret` call
# piped through ONE `kubectl apply`: a second, separate `kubectl create
# secret X --dry-run=client -o yaml | kubectl apply -f -` for just a few
# keys REPLACES the whole Secret object rather than merging into it, and
# silently deletes every key it didn't mention -- that's a real mistake this
# comment exists to prevent a repeat of (it happened once, mid-September
# 2026, and required a full stack wipe to recover from). Add new secret
# keys to this one invocation, never as a follow-up apply.
#
...
# Cloudflare Realtime TURN credentials: long-lived account credentials plus
# a short-lived generated pair, passed in via env (not generated here -- see
# deploy/kubernetes/turn-refresh.sh) and included as optional literals so a
# re-run of this script doesn't clobber them if they're already set.
CF_TURN_ARGS=()
[ -n "${CLOUDFLARE_TURN_KEY_ID:-}" ] && CF_TURN_ARGS+=(--from-literal=CLOUDFLARE_TURN_KEY_ID="$CLOUDFLARE_TURN_KEY_ID")
[ -n "${CLOUDFLARE_TURN_API_TOKEN:-}" ] && CF_TURN_ARGS+=(--from-literal=CLOUDFLARE_TURN_API_TOKEN="$CLOUDFLARE_TURN_API_TOKEN")
[ -n "${CLOUDFLARE_TURN_USERNAME:-}" ] && CF_TURN_ARGS+=(--from-literal=CLOUDFLARE_TURN_USERNAME="$CLOUDFLARE_TURN_USERNAME")
[ -n "${CLOUDFLARE_TURN_CREDENTIAL:-}" ] && CF_TURN_ARGS+=(--from-literal=CLOUDFLARE_TURN_CREDENTIAL="$CLOUDFLARE_TURN_CREDENTIAL")
