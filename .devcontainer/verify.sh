...
if [ "$SKIP_INSTALL" -eq 0 ]; then
	stage "bun install" bun install --frozen-lockfile
else
	echo
	echo "=== bun install === (skipped)"
fi

stage "wasm: bun --filter fluxer_app wasm:codegen" bun --filter fluxer_app wasm:codegen

stage "app: typecheck" bun --filter fluxer_app typecheck
stage "app: unit tests" bash -c 'cd fluxer_app && bunx vitest run'

if [ "$QUICK" -eq 0 ]; then
	stage "desktop: typecheck" bun --filter fluxer_desktop typecheck
	stage "app: production build" bun --filter fluxer_app build
fi
